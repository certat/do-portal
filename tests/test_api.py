...
from tests import DOTestCase, create_sample
from unittest.mock import patch, MagicMock


class APITestCase(DOTestCase):
    ...
    def create_deliverable(self):
        return self.client.post(
            ...
            data=json.dumps(dict(name="Sample deliverable")),
            headers=self.get_req_headers()
        )

    def test_404(self):
        rv = self.client.get('/api/1.0/non-existent-resource')
        self.assertTrue(rv.status_code == 404)

    def test_organizations(self):
        rv = self.client.get(
            url_for('api.get_organizations'),
            headers=self.get_req_headers()
        )
        ...
        # get organization
        rv = self.client.get(
            url_for('api.get_organization', org_id=1),
            headers=self.get_req_headers()
        )
        self.assertJSONIsNotNone(rv, key='abbreviation')
        ...
        self.assertTrue(rv.status_code == 501)
